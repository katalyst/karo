...
module Karo
    ...
    def sync_server_to_local_database(server_db_config, local_db_config)
      ...
      when "postgresql"
        "#{ssh} \"export PGPASSWORD='#{server_db_config["password"]}'; pg_dump -Fc -U #{server_db_config["username"]} -h #{server_db_config["host"]} #{server_db_config["database"]} | gzip -9 -c\" | gunzip -c | pg_restore -h #{local_db_config["host"]} -U #{local_db_config["username"]} -d #{local_db_config["database"]} --clean" 
      else
        raise Thor::Error, "Please make sure that the database adapter is either mysql2 or postgresql?"
      end
